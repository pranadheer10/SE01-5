...
public class Semester_Data_Importing {
	JFrame frame;
	JPanel mainpanle;
	JButton submit;
	JButton cancel;
	...
	public Semester_Data_Importing() {
		frame = new JFrame();
		...
		liftLable = new JLabel("Import Semester Information");
		viewLable = new JLabel("View");
		import_B = new JButton("Import");
		importLable = new JLabel("Click On Import To Add Semester File");
		...
		mainLable = new JLabel("Semester Data Importing");
		...
		for (SemesterDM semesterDM : SemesterService.AllSemestersDM) {
			// System.out.println(studentDM.dataprint());
			alldata = alldata + semesterDM.printdata();

			model.addElement(semesterDM.printdata2());
	...
	ActionListener deleteAction = new ActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (dataArea.getSelectedValue() != null) {
				...
				SemesterService.AllSemestersDM = null;
				new SemesterService().loadSemestersLists();
				String alldata = "";
				// JOptionPane.showMessageDialog(null,
				// StudentService.AllStdeuntsDM.size());
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (dataArea.getSelectedValue() != null) {
				try {
					...
					frame.setVisible(false);
					new Edit_Data_Semester(
							findSemesterRecord(dataArea.getSelectedValue()));
				} catch (IOException e1) {
	...
	public SemesterDM findSemesterRecord(String s) {
		for (SemesterDM semesterDM : SemesterService.AllSemestersDM) {
			String selecteddata[] = s.split(" - ");
			if (semesterDM.getSemesterName().equals(selecteddata[0])
					&& semesterDM.getStart_Date().equals(selecteddata[1])) {
				return semesterDM;
			}
		}
		return null;
	...
	public int getIndexOFstudent(SemesterDM semester) {
		int x = 0;

		for (SemesterDM semesterDM : SemesterService.AllSemestersDM) {
			x++;
			if (semesterDM == semester) {
				return x;
			}
		}
		return -1;
	...
	public void removeanStuden(String lineToRemove) throws IOException {
		...
		File inputFile = new File("TestSemesters.txt");
